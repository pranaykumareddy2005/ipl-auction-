...
function applyEvent(s, ev) {
  ...
  switch (ev.type) {
    case EVENTS.AUCTION_CONFIGURED: {
      // Only accept known settings, clamped to sane ranges (ignore unknown keys / junk).
      const cur = s.settings, inc = d.settings || {};
      const clamp = (v, def, min, max) => { const n = Number(v); return Number.isFinite(n) ? Math.min(max, Math.max(min, Math.round(n))) : def; };
      s.settings = {
        minTeams: cur.minTeams,
        purseL: inc.purseL != null ? clamp(inc.purseL, cur.purseL, 1, 100000000) : cur.purseL,
        squadMax: inc.squadMax != null ? clamp(inc.squadMax, cur.squadMax, 1, 100) : cur.squadMax,
        overseasMax: inc.overseasMax != null ? clamp(inc.overseasMax, cur.overseasMax, 0, 100) : cur.overseasMax,
        timerSec: inc.timerSec != null ? clamp(inc.timerSec, cur.timerSec, 3, 3600) : cur.timerSec,
      };
      break;
    }
    ...
    case EVENTS.TEAMS_SET:
      s.teams = (d.teams || []).map((t, i) => ({
        ...
        short: String(t.short || (t.name ? t.name.slice(0, 3).toUpperCase() : `T${i + 1}`)).slice(0, 6),
        // Only accept a valid #hex color; anything else falls back — prevents style-
        // attribute injection when the color is rendered into inline styles.
        color: /^#[0-9a-fA-F]{3,8}$/.test(t.color) ? t.color : '#888',
        owner: String(t.owner || '').slice(0, 40),
        purseL: (() => { const n = Number(t.purseL); return Number.isFinite(n) && n >= 0 ? Math.round(n) : s.settings.purseL; })(),
      }));
      break;
